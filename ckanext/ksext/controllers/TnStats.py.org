...
WHERE  package_id=%s, user_id=%s
    ...
    def index (self):
        ...
        _ViewCount = collections.namedtuple("ViewCount", "id title name org_name dataset_views resource_views resource_downloads api_views")
...
ORDER BY
  ...
  p.title ASC; '''
    ...
    def orgCsv(self):
        head = u'\ufeff組織,資料集,資料集編碼,資料集瀏覽次數,資料瀏覽次數,資料下載次數,API存取次數\r\n'
        data = self._orgApiResult()
    ...
    def _csv(self, head, data):
        ...
        csvFormatter = u'"{0}","{1}","{2}",{3},{4},{5},{6}\r\n'
        for item in data:
            #data += csvFormatter.format(*item)
            head += csvFormatter.format(item[3],item[1],item[2],item[4],item[5],item[6],item[7])
        return head
...
SELECT p.id, p.title, p.name, pe.value AS freq, p.owner_org AS org_id, g.title AS org_name,
	(SELECT MAX(openness_score) FROM qa where package_id=p.id ) AS open_stars,
